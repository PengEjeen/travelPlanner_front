  ...
  <div>
    <div class="festival-search">
      <div class="festival-inputBox">
        <input type="text" id="areaSearch" v-model="searchText" placeholder="축제 지역 검색">
      ...
      <div class="festival-dropdown-group">
        ...
        <button type="button" class="festival-search-button2" @click="fetchData">검색</button>
      </div>
    </div>

    <div class="festival-results">
      <div v-for="item in items" :key="item.id" class="festival-result-item" @click="openModal(item)">
    ...
    <!-- 모달 -->
    <div v-if="isModalOpen" class="festival-modal-overlay" @click="closeModal">
      <div class="festival-modal" @click.stop>
        <button class="festival-close-button" @click="closeModal">&times;</button>
        <div class="festival-modal-content">
          <img :src="selectedItem.imageUrl" alt="축제 이미지" class="festival-modal-image">
          <div class="festival-modal-info">
            <h3>{{ selectedItem.title }}</h3>
            <p>기간: {{ selectedItem.date }}</p>
            <p>위치: {{ selectedItem.location }}</p>
            <p>전화번호: {{ selectedItem.phone }}</p>
            <p>홈페이지: <a :href="selectedItem.website" target="_blank">{{ selectedItem.website }}</a></p>
          </div>
        </div>
        <p class="festival-modal-description">설명: {{ selectedItem.description }}</p>
      </div>
    </div>
  </div>
</template>
  ...
  items.value = [
    {
      ...
      location: '서울',
      imageUrl: 'https://minio.nculture.org/amsweb-opt/multimedia_assets/73/30127/16061/c/026_2019정선아리랑제-thumb.jpg',
      phone: '02-123-4567',
...
const fetchData = async () => {
  ...
  const response = await fetch(`https://api.example.com/festivals?month=${month}&city=${city}`) // 예시 URL
...
const fetchDataByText = async () => {
  ...
  const response = await fetch(`https://api.example.com/festivals?area=${area}`) // 예시 URL
  const data = await response.json()
  items.value = data.results
}
